...
document.addEventListener("DOMContentLoaded", (event) => {
    ...
    items.forEach(function (el) {
        ...
        let mouseMoveTimer; //마우스가 멈출 때를 감지하는 변수
    ...
    list.forEach(function (el) {
        ...
        el.addEventListener("mouseenter", function (event) {
            ...
            // setInterval을 멈추고 싶다면 setInerval을 변수에 할당
            // let interval=setInterval(할일,시간)
            // 멈출 때 clearInterval(변수) //clearInterval(interval)
            // console.log(target_element.innerText)
            // target_element.innerText="메롱"
            // console.log(target_element.dataset.value[0])
            // .map(function(){ //map함수는 배열 안에 요소를 하나씩 불러서 할일 // 새로운 배열을 만든다 //item은 요소를 하나씩 받음 // index는 그 요소의 index 번호
            //     return
            // })
            ...
            let interval = setInterval(function () {
                ...
                iteration += 1 / 3; //interation = iteration + 1/3
            }, 20)
        })
    })
});